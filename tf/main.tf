...
# Defaults
#   - 1 RHEL 8 Main (used to SSH into to install AAP)
...
resource "openstack_compute_instance_v2" "main" {
  name = "${var.username}_aap_${var.aap_version}_main"
  image_name = "RHEL-8.6.0-x86_64-released"
  flavor_name = "ci.m1.micro"
  ...
  metadata = {
    description = "Used for SSHing into to install AAP on the other servers"
  }
}
